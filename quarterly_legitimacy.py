...
def run_quarterly():
    ...
    quarterly = [row for row in data if (dt.date.fromisoformat(row["timestamp"][:10]).month - 1)//3 + 1 == quarter]
    if not quarterly:
        print("No quarterly data.")
        return
    fig = px.area(quarterly, x="timestamp", y="kpi_value", color="role",
                  title="Quarterly Legitimacy Snapshot", template="plotly_dark")
    filename = "quarterly_legitimacy.html"
    upload_chart(fig, filename)
    insert_dashboard_link("quarterly", filename)
    notify_stakeholders("quarterly", filename)
