...
export default function CosmicLedgerCouncilGovernanceDashboard() {
  ...
  useEffect(() => {
    async function loadCouncil() {
      const { data, error } = await supabase.rpc(
        "ledger_council_governance_dashboard"
      );

      if (error) console.error("Council Governance RPC error:", error);

      setCouncil(data || null);
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <CosmicPage className="cosmic-ledger">
        <CosmicHeader title="Council Governance Dashboard" />
        <div className="p-6 text-slate-200">Loading council governance…</div>
      </CosmicPage>
    );
  }
  ...
  if (!council) {
    return (
      <CosmicPage className="cosmic-ledger">
        <CosmicHeader title="Council Governance Dashboard" />
        <div className="p-6 text-slate-200">No council governance data.</div>
      </CosmicPage>
    );
  }
  ...
  return (
    <CosmicPage className="cosmic-ledger">
      ...
      {/* Council Governance Dashboard */}
      <div className="p-6 text-slate-200">

        <h1 className="text-2xl font-bold mb-6">Council Governance Metrics</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            ...
            <p className="text-3xl mt-2">{council.council_count}</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Governance Score</h2>
            <p className="text-3xl mt-2">{council.governance_score}%</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Financial Stewardship</h2>
            <p className="text-3xl mt-2">{council.stewardship_score}%</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Active Governance Flags</h2>
            <p className="text-3xl mt-2">{council.flags}</p>
          </div>

        </div>

        {/* Council Breakdown */}
        <h2 className="text-xl font-bold mb-2">Council Breakdown</h2>
        <div className="bg-slate-800 p-4 rounded mb-6">
          {council.council_breakdown.map((c, i) => (
            <div key={i} className="flex items-center gap-4 mb-2">
              <span className="w-40 text-slate-300">{c.council_name}</span>
        ...
        {/* Governance Flags */}
        <h2 className="text-xl font-bold mb-2">Governance Flags</h2>
        <div className="bg-slate-800 p-4 rounded mb-6">
          {council.flag_list.map((f, i) => (
      ...
      {/* Cosmic Council Governance Layer */}
      <div className="space-y-12 mt-12">

        <CosmicMinistrySpirit name="Council Governance Spirit" />
        <CosmicMinistryThrone name="Council Governance Throne" />
        <CosmicMinistryThroneAscended name="Council Governance Throne — Ascended" />
        <CosmicMinistryThroneEternal name="Council Governance — Eternal Throne" />
        ...
        <CosmicDivineBeing name="Council Governance — Divine Being" />
