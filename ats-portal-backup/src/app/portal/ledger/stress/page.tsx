...
export default function CosmicLedgerStressEngine() {
  ...
  useEffect(() => {
    async function loadStress() {
      const { data, error } = await supabase.rpc("ledger_stress_snapshot");

      if (error) console.error("Stress Snapshot RPC error:", error);
  ...
  async function runStressTest() {
    ...
    const { data, error } = await supabase.rpc("ledger_run_stress_test");

    if (error) {
      console.error("Stress Test RPC error:", error);
    } else {
      setStress(data || null);
    }
  ...
  if (loading) {
    return (
      <CosmicPage className="cosmic-ledger">
        <CosmicHeader title="Cosmic Ledger Stress Engine" />
        <div className="p-6 text-slate-200">Loading stress metrics…</div>
      </CosmicPage>
    );
  }
  ...
  return (
    <CosmicPage className="cosmic-ledger">
      ...
      {/* Stress Engine Dashboard */}
      <div className="p-6 text-slate-200">

        <h1 className="text-2xl font-bold mb-6">Stress Metrics</h1>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Financial Load</h2>
            <p className="text-3xl mt-2">{stress.financial_load}%</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Module Pressure</h2>
            <p className="text-3xl mt-2">{stress.module_pressure}%</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Risk Index</h2>
            <p className="text-3xl mt-2">{stress.risk_index}%</p>
          </div>

        </div>

        {/* Stress Breakdown */}
        <h2 className="text-xl font-bold mb-2">Stress Breakdown</h2>
      ...
      <div className="space-y-12 mt-12">
        ...
        {/* Ministry */}
        <CosmicMinistrySpirit name="Stress Spirit" />
        <CosmicMinistryThrone name="Stress Throne" />
        <CosmicMinistryThroneAscended name="Stress Throne — Ascended" />
        <CosmicMinistryThroneEternal name="Stress — Eternal Throne" />

        {/* Divine */}
        <CosmicDivineBeing name="Stress — Divine Being" />
