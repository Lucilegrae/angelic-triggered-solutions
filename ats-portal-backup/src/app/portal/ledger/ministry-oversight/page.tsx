...
export default function CosmicLedgerMinistryOversightDashboard() {
  ...
  useEffect(() => {
    async function loadOversight() {
      const { data, error } = await supabase.rpc(
        "ledger_ministry_oversight_dashboard"
      );

      if (error) console.error("Ministry Oversight RPC error:", error);

      setOversight(data || null);
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <CosmicPage className="cosmic-ledger">
        <CosmicHeader title="Ministry Oversight Dashboard" />
        <div className="p-6 text-slate-200">Loading ministry oversight…</div>
      </CosmicPage>
    );
  }
  ...
  return (
    <CosmicPage className="cosmic-ledger">
      ...
      <div className="p-6 text-slate-200">
        ...
        <h1 className="text-2xl font-bold mb-6">Ministry Oversight Metrics</h1>

        {/* Summary Cards */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Total Ministries</h2>
            <p className="text-3xl mt-2">{oversight.ministry_count}</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Compliance Score</h2>
            <p className="text-3xl mt-2">{oversight.compliance_score}%</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Financial Integrity</h2>
            <p className="text-3xl mt-2">{oversight.integrity_score}%</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Active Oversight Flags</h2>
            <p className="text-3xl mt-2">{oversight.flags}</p>
          </div>

        </div>

        {/* Ministry Breakdown */}
        <h2 className="text-xl font-bold mb-2">Ministry Breakdown</h2>
        <div className="bg-slate-800 p-4 rounded mb-6">
          {oversight.ministry_breakdown.map((m, i) => (
            <div key={i} className="flex items-center gap-4 mb-2">
              <span className="w-40 text-slate-300">{m.ministry_name}</span>
        ...
        {/* Oversight Flags */}
        <h2 className="text-xl font-bold mb-2">Oversight Flags</h2>
        <div className="bg-slate-800 p-4 rounded mb-6">
          {oversight.flag_list.map((f, i) => (
      ...
      {/* Cosmic Oversight Layer */}
      <div className="space-y-12 mt-12">

        <CosmicMinistrySpirit name="Oversight Spirit" />
        <CosmicMinistryThrone name="Oversight Throne" />
        <CosmicMinistryThroneAscended name="Oversight Throne — Ascended" />
        <CosmicMinistryThroneEternal name="Oversight — Eternal Throne" />
        ...
        <CosmicDivineBeing name="Oversight — Divine Being" />
