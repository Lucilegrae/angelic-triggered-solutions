...
export default function MinistryDashboard() {
  ...
  const filteredCoverage = coverage?safe().filter(
    (c: any) =>
  ...
  const filteredTiers = safe(safeTiers).filter((t: any) =>
    !filters.tier || t.tier === filters.tier
  );

  const safeEstates = Array.isArray(estates) ? estates : [];
  const filteredEstates = safe(safeEstates).filter((e: any) =>
    (!filters.block ||
  ...
  const filteredRisks = safe(safeRisks).filter((r: any) =>
  ...
  const filteredLifecycle = safe(safeLifecycle).filter((l: any) =>
    ...
    <div className="p-6 space-y-6">
      ...
      {!loading && (
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiWidget
            label="Total Members"
            value={filteredCoverage.length}
          />

          <KpiWidget
            label="Total Estates"
            value={filteredEstates.length}
            color="blue"
          />
          ...
          <KpiWidget
            label="High Risk"
            value={
              safe(filteredRisks).filter(
                (r: any) => r.risk_level === "High Risk"
              ).length
            }
            color="red"
          />
          ...
          <KpiWidget
            ...
            value={filteredLifecycle.length}
            color="amber"
          />
        </section>
      )}
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {filteredEstates.map((e: any, idx: number) => (
                <EstatePressureCard key={e.id ?? idx} {...e} />
              ))}
            </div>
          </section>
