...
export default function MemberIntelligencePage() {
  ...
  const member = coverage?.find(
    (m: any) => m.national_id === id
  );

  const memberLifecycle = lifecycle?.filter(
    (l: any) => l.national_id === id
  );
      ...
      <section className="space-y-2">
        ...
        {result && (
          <div className="mt-3 text-xs border border-red-500/40 rounded-lg p-3 bg-slate-900/40">
            <p>
              <strong>Score:</strong> {result.prediction.score}
            </p>

            <p>
              <strong>Band:</strong> {result.prediction.band}
            </p>
          </div>
        )}
      </section>
