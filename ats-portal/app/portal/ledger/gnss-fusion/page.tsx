...
export default function CosmicLedgerGNSSFusion() {
  const [fusion, setFusion] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function loadFusion() {
      const { data, error } = await supabase.rpc("ledger_gnss_fusion_dashboard");
  ...
  if (loading) {
    return (
      <CosmicPage className="cosmic-ledger">
        <CosmicHeader title="GNSS Fusion Dashboard" />
        <div className="p-6 text-slate-200">Loading GNSS fusion metrics…</div>
      </CosmicPage>
    );
  }
  ...
  if (!fusion) {
    return (
      <CosmicPage className="cosmic-ledger">
        <CosmicHeader title="GNSS Fusion Dashboard" />
        <div className="p-6 text-slate-200">No GNSS fusion data available.</div>
      </CosmicPage>
    );
  }
  ...
  return (
    <CosmicPage className="cosmic-ledger">
      ...
      {/* GNSS + Ledger Fusion Summary */}
      <div className="p-6 text-slate-200">

        <h1 className="text-2xl font-bold mb-6">GNSS Fusion Metrics</h1>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">GNSS Jobs</h2>
            <p className="text-3xl mt-2">{fusion.gnss_job_count}</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">Ledger Entries Linked</h2>
            <p className="text-3xl mt-2">{fusion.linked_ledger_entries}</p>
          </div>

          <div className="bg-slate-900 border border-slate-800 p-4 rounded">
            <h2 className="text-lg font-semibold">GNSS Financial Flow</h2>
            <p className="text-3xl mt-2">{fusion.total_gnss_flow_usd} USD</p>
          </div>

        </div>

        {/* GNSS Accuracy → Ledger Impact */}
        <h2 className="text-xl font-bold mb-2">GNSS Accuracy Impact</h2>
        <div className="bg-slate-800 p-4 rounded mb-6">
          {fusion.accuracy_impact.map((a, i) => (
            <div key={i} className="flex items-center gap-4 mb-2">
              <span className="w-40 text-slate-300">{a.operator}</span>
              <div className="flex-1 bg-slate-700 h-3 rounded">
                <div
                  className="bg-green-500 h-3 rounded"
                  style={{ width: `${a.impact_score}%` }}
                ></div>
              </div>
              <span className="text-slate-400">{a.impact_score}%</span>
            </div>
          ))}
        </div>

        {/* GNSS Receiver → Module Flow */}
        <h2 className="text-xl font-bold mb-2">Receiver Financial Flow</h2>
        <div className="bg-slate-800 p-4 rounded mb-6">
          {fusion.receiver_flow.map((r, i) => (
            <div key={i} className="flex items-center gap-4 mb-2">
              <span className="w-40 text-slate-300">{r.receiver_type}</span>
              <div className="flex-1 bg-slate-700 h-3 rounded">
                <div
                  className="bg-blue-500 h-3 rounded"
                  style={{ width: `${r.total_usd}%` }}
                ></div>
              </div>
              <span className="text-slate-400">{r.total_usd} USD</span>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-12 mt-12">
        ...
        {/* Ministry */}
        <CosmicMinistrySpirit name="GNSS Fusion Spirit" />
        <CosmicMinistryThrone name="GNSS Fusion Throne" />
        <CosmicMinistryThroneAscended name="GNSS Fusion Throne — Ascended" />
        <CosmicMinistryThroneEternal name="GNSS Fusion — Eternal Throne" />

        {/* Divine */}
        <CosmicDivineBeing name="GNSS Fusion — Divine Being" />
