...
def run_daily():
    data = fetch_data()
    today = dt.date.today()
    daily = [row for row in data if dt.date.fromisoformat(row["timestamp"][:10]) == today]
    if not daily:
        print("No daily data.")
        return
    fig = px.line(daily, x="timestamp", y="kpi_value", color="role",
                  title="Daily Legitimacy Snapshot", template="plotly_dark")
    filename = "daily_legitimacy.html"
    upload_chart(fig, filename)
    insert_dashboard_link("daily", filename)
    notify_stakeholders("daily", filename)
