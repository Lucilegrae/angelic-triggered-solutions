...
for (let currentModule = module;;) {
    if (FLAT_CONFIG_REGEX.test(currentModule.filename)) {
        ...
        const targetFrame = stackTrace.find((frame) => frame.file && frame.file.endsWith('eslint.jsx'));
        if (targetFrame) {
    ...
    for (let currentModule = module;;) {
        if (!configArrayFactoryPath) {
            ...
            try {
                ...
                const resolvedConfigArrayFactoryPath = node_path_1.default.join(eslintrcFolder, '/lib/config-array-factory.jsx');
        ...
        else if (currentModule.filename.endsWith('cli-engine.jsx')) {
            // Next look for a file in ESLint's folder
            //   .../eslint/lib/cli-engine/cli-engine.js
            try {
                const eslintCandidateFolder = node_path_1.default.dirname(require.resolve('eslint/package.json', {
                ...
                if (node_path_1.default.join(eslintCandidateFolder, 'lib/cli-engine/cli-engine.jsx') === currentModule.filename) {
                    exports.eslintFolder = eslintFolder = eslintCandidateFolder;
                    break;
                }
            }
