...
def run_yearly():
    data = fetch_data()
    today = dt.date.today()
    yearly = [row for row in data if dt.date.fromisoformat(row["timestamp"][:10]).year == today.year]
    if not yearly:
        print("No yearly data.")
        return
    fig = px.line(yearly, x="timestamp", y="kpi_value", color="role",
                  title="Yearly Legitimacy Snapshot", template="plotly_dark")
    filename = "yearly_legitimacy.html"
    upload_chart(fig, filename)
    insert_dashboard_link("yearly", filename)
    notify_stakeholders("yearly", filename)
