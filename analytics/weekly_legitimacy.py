...
def run_weekly():
    ...
    weekly = [row for row in data if dt.date.fromisoformat(row["timestamp"][:10]) >= start_week]
    if not weekly:
        print("No weekly data.")
        return
    fig = px.bar(weekly, x="timestamp", y="kpi_value", color="role",
                 title="Weekly Legitimacy Snapshot", template="plotly_dark")
    filename = "weekly_legitimacy.html"
    upload_chart(fig, filename)
    insert_dashboard_link("weekly", filename)
    notify_stakeholders("weekly", filename)
